"""
Fine-tune the pretrained segmentation model with focal loss.

Loads the trial1 autoencoder checkpoint, attaches a fresh U-Net decoder,
then trains with Focal loss for tissue segmentation.

Reuses outputs_trial1/autoencoder/best_autoencoder.pth (trained at 256x256).

Usage:
    conda run -n ML python3 src/train_pretrained_seg_focal.py

Saves to: outputs_focal/pretrained_seg/
  - best_model.pth
  - last_model.pth
  - training_log.json
"""
...
OUTPUT_DIR  = pathlib.Path("outputs_focal/pretrained_seg")
...
# Focal loss uses class weights to keep the minority classes visible.
