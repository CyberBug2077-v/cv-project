"""
Fine-tune segmentation from the masked-MSE autoencoder using Dice loss.

Loads the autoencoder trained with masked/corrupted reconstruction (MSE loss),
attaches a fresh U-Net segmentation decoder, and fine-tunes end-to-end with
pure Dice loss.  This is otherwise identical to Trial 4 (standard pretrained
+ Dice), allowing a direct comparison of pretraining quality.

Usage:
    conda run -n ML python3 src/train_pretrained_seg_masked_mse_dice.py

Saves to: outputs_masked_mse/pretrained_seg/
  - best_model.pth
  - last_model.pth
  - training_log.json
"""
...
BATCH_SIZE = 8
LR         = 1e-4
AE_CKPT    = pathlib.Path("outputs_masked_mse/autoencoder/best_autoencoder.pth")
OUTPUT_DIR = pathlib.Path("outputs_masked_mse/pretrained_seg")
