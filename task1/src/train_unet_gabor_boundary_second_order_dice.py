"""
Train the second-order boundary-aware Gabor U-Net with Dice and boundary supervision.

Usage:
    conda run -n ML python3 src/train_unet_gabor_boundary_second_order_dice.py

Saves to: outputs_gabor_boundary_second_order_dice/unet/
  - best_model.pth
  - last_model.pth
  - training_log.json
"""
...
from model_unet import BoundaryAwareSecondOrderGaborUNet, count_parameters
...
OUTPUT_DIR = pathlib.Path("outputs_gabor_boundary_second_order_dice/unet")
...
model = BoundaryAwareSecondOrderGaborUNet(in_channels=3, num_classes=NUM_CLASSES).to(device)
