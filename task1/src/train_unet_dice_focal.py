"""
Train the U-Net with Dice plus focal loss.

Combined loss = DiceLoss + FocalLoss(gamma=1.0, weight=class_weights).
Dice handles per-class balance; Focal with mild gamma provides per-pixel
gradient signal and up-weights minority-class pixels via alpha.

Usage:
    conda run -n ML python3 src/train_unet_dice_focal.py

Saves to: outputs_dice_focal/unet/
  - best_model.pth
  - last_model.pth
  - training_log.json
"""
...
LR         = 1e-4
GAMMA      = 1.0
OUTPUT_DIR = pathlib.Path("outputs_dice_focal/unet")
