"""
Fine-tune the segmentation model from the plain autoencoder checkpoint.

Loads the best autoencoder checkpoint, attaches a fresh U-Net decoder,
then trains the full model for tissue segmentation.

The encoder is NOT frozen — the entire network fine-tunes from the
pretrained initialisation. This gives the encoder a head start without
restricting its ability to adapt.

Usage:
    conda run -n ML python3 src/train_pretrained_seg.py

Saves to: outputs/pretrained_seg/
  - best_model.pth
  - last_model.pth
  - training_log.json
"""
...
BATCH_SIZE  = 4
LR          = 1e-4
AE_CKPT     = pathlib.Path("outputs_trial2/autoencoder/best_autoencoder.pth")
OUTPUT_DIR  = pathlib.Path("outputs_trial2/pretrained_seg")
...
# Use weighted cross-entropy for the supervised fine-tuning stage.
