"""
Train the masked-reconstruction autoencoder with a balanced MSE and L1 loss.

Same corruption strategy as train_autoencoder_masked_mse.py.  The difference
is the reconstruction loss: a balanced combination of MSE and L1.

  MSE alone: penalises large errors quadratically, smooth gradients, but can
             produce blurry reconstructions as it averages over uncertainty.
  L1 alone:  penalises all errors linearly, sharper reconstructions, but
             noisier gradients near zero.
  0.5·MSE + 0.5·L1: combines quadratic sensitivity to large errors (MSE)
             with robustness to outlier pixels (L1).  In pathology images this
             may better preserve sharp tissue boundaries versus MSE alone.

Corruption applied per image:
  1. Random block masking (always): 4–8 blocks of 24–48 px, filled with 0.0.
  2. Gaussian noise (p=0.7): σ=0.15.
  3. Stain jitter (p=0.7): per-channel scale in [0.8, 1.2].

Reconstruction target: always the clean, unmodified image.

Usage:
    conda run -n ML python3 src/train_autoencoder_masked_mse_l1.py

Saves to: outputs_masked_mse_l1/autoencoder/
...
BATCH_SIZE = 8
LR         = 1e-4
OUTPUT_DIR = pathlib.Path("outputs_masked_mse_l1/autoencoder")
...
def criterion(recon, target):
    return 0.5 * mse_loss(recon, target) + 0.5 * l1_loss(recon, target)
...
print(f"\nMasked MSE+L1 pretraining complete. Best val loss: {best_val_loss:.5f}")
