"""
Fine-tune the pretrained segmentation model with soft Dice loss.

Loads the trial1 autoencoder checkpoint, attaches a fresh U-Net decoder,
then trains with Dice loss for tissue segmentation.

Reuses outputs_trial1/autoencoder/best_autoencoder.pth (trained at 256x256).

Usage:
    conda run -n ML python3 src/train_pretrained_seg_dice.py

Saves to: outputs_dice/pretrained_seg/
  - best_model.pth
  - last_model.pth
  - training_log.json
"""
...
OUTPUT_DIR  = pathlib.Path("outputs_dice/pretrained_seg")
