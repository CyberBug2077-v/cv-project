"""
Train the first-order Gabor U-Net with Dice plus cross-entropy loss.

This keeps the standard U-Net training setup unchanged apart from adding a
first-order Gabor competition feature map at the first encoder stage.

Usage:
    conda run -n ML python3 src/train_unet_gabor_dice_ce.py

Saves to: outputs_gabor_dice_ce/unet/
  - best_model.pth
  - last_model.pth
  - training_log.json
"""
...
from model_unet import GaborCompetitionUNet, count_parameters
...
BATCH_SIZE = 8
LR         = 1e-4
OUTPUT_DIR = pathlib.Path("outputs_gabor_dice_ce/unet")
...
# Build the Gabor-augmented U-Net.

model = GaborCompetitionUNet(in_channels=3, num_classes=NUM_CLASSES).to(device)
...
# Dice and weighted cross-entropy are summed with equal weight.
