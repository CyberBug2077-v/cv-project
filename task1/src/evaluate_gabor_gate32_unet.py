"""
Compare the Dice+CE baseline U-Net against the gated 32-channel Gabor U-Net.

Usage:
    conda run -n ML python3 src/evaluate_gabor_gate32_unet.py <output_dir>
"""
...
from model_unet import UNet, GatedGaborCompetitionUNet, count_parameters
from metrics import ConfusionMatrix, format_metrics
from visualise import save_comparison, save_legend

BASELINE_CKPT = pathlib.Path("outputs_dice_ce/unet/best_model.pth")

if len(sys.argv) < 2:
    print("Usage: python3 src/evaluate_gabor_gate32_unet.py <output_dir>")
    sys.exit(1)
...
def load_gabor_unet(ckpt_path):
    model = GatedGaborCompetitionUNet(in_channels=3, num_classes=NUM_CLASSES)
    model.load_state_dict(torch.load(ckpt_path, map_location="cpu"))
    model.to(device).eval()
    return model
...
def save_hard_samples(gabor_results, baseline_results, gabor_model, baseline_model):
    ...
    for result in sorted_results[:hard_n]:
        ...
        save_comparison(
            ...
            hard_dir / f"{name}_gabor_gate32_unet.png",
            title=f"Gabor gate32 U-Net dice={result['mean_dice']:.3f}",
        )
        save_comparison(
        ...
        hard_records.append({
            "name":               name,
            "gabor_gate32_dice":  result["mean_dice"],
...
if __name__ == "__main__":
    ...
    print(f"Gabor gate32 U-Net params: {count_parameters(gabor_model):,}")

    base_val_m, base_val_r = evaluate_model(baseline_model, "baseline_unet", "validation")
    gabor_val_m, gabor_val_r = evaluate_model(gabor_model, "gabor_gate32_unet", "validation")

    base_test_m, base_test_r = evaluate_model(baseline_model, "baseline_unet", "test")
    gabor_test_m, gabor_test_r = evaluate_model(gabor_model, "gabor_gate32_unet", "test")

    print("\n=== Hard Sample Analysis (10 lowest Gabor gate32 U-Net Dice on test set) ===")
    ...
    for label, bm, gm in [("Validation", base_val_m, gabor_val_m),
        ...
        print(f"    Gabor gate32 U-Net  Dice={gm['mean_dice']:.4f}  IoU={gm['mean_iou']:.4f}  PixAcc={gm['pixel_accuracy']:.4f}")
