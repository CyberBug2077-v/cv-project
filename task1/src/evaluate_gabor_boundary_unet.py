"""
Compare the Dice baseline U-Net against the boundary-aware Gabor U-Net.

Usage:
    conda run -n ML python3 src/evaluate_gabor_boundary_unet.py <output_dir>
"""
...
if len(sys.argv) < 2:
    print("Usage: python3 src/evaluate_gabor_boundary_unet.py <output_dir>")
    sys.exit(1)
...
def save_hard_samples(gabor_results, baseline_results, gabor_model, baseline_model):
    ...
    for result in sorted_results[:hard_n]:
        ...
        save_comparison(
            ...
            hard_dir / f"{name}_gabor_boundary_unet.png",
            title=f"Gabor boundary U-Net dice={result['mean_dice']:.3f}",
        )
        save_comparison(
        ...
        hard_records.append({
            "name": name,
            "gabor_boundary_dice": result["mean_dice"],
...
if __name__ == "__main__":
    ...
    print(f"Gabor boundary U-Net params : {count_parameters(gabor_model):,}")

    base_val_m, base_val_r = evaluate_model(baseline_model, "baseline_unet", "validation")
    gabor_val_m, gabor_val_r = evaluate_model(gabor_model, "gabor_boundary_unet", "validation")

    base_test_m, base_test_r = evaluate_model(baseline_model, "baseline_unet", "test")
    gabor_test_m, gabor_test_r = evaluate_model(gabor_model, "gabor_boundary_unet", "test")

    print("\n=== Hard Sample Analysis (10 lowest Gabor boundary U-Net Dice on test set) ===")
    ...
    for label, bm, gm in [("Validation", base_val_m, gabor_val_m),
        ...
        print(f"    Gabor boundary U-Net  Dice={gm['mean_dice']:.4f}  IoU={gm['mean_iou']:.4f}  PixAcc={gm['pixel_accuracy']:.4f}")
