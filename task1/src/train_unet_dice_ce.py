"""
Train the U-Net with Dice plus cross-entropy loss.

Combined loss = DiceLoss + CrossEntropyLoss(weight=class_weights).
Dice handles per-class balance; weighted CE provides stable per-pixel
gradient signal with explicit class-frequency correction via alpha.

Usage:
    conda run -n ML python3 src/train_unet_dice_ce.py

Saves to: outputs_dice_ce/unet/
  - best_model.pth
  - last_model.pth
  - training_log.json
"""
...
BATCH_SIZE = 8
LR         = 1e-4
OUTPUT_DIR = pathlib.Path("outputs_dice_ce/unet")
