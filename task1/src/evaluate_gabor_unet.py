"""
Compare the Dice+CE baseline U-Net against the first-order Gabor U-Net.

Usage:
    conda run -n ML python3 src/evaluate_gabor_unet.py <output_dir>

Requires:
    outputs_dice_ce/unet/best_model.pth
...
from model_unet import UNet, GaborCompetitionUNet, count_parameters
...
if len(sys.argv) < 2:
    print("Usage: python3 src/evaluate_gabor_unet.py <output_dir>")
    sys.exit(1)
...
def load_gabor_unet(ckpt_path):
    model = GaborCompetitionUNet(in_channels=3, num_classes=NUM_CLASSES)
    model.load_state_dict(torch.load(ckpt_path, map_location="cpu"))
    model.to(device).eval()
    return model
...
def save_hard_samples(gabor_results, baseline_results, gabor_model, baseline_model):
    ...
    for result in sorted_results[:hard_n]:
        ...
        save_comparison(
            ...
            hard_dir / f"{name}_gabor_unet.png",
            title=f"Gabor U-Net dice={result['mean_dice']:.3f}",
        )
        save_comparison(
        ...
        hard_records.append({
            "name":              name,
            "gabor_unet_dice":   result["mean_dice"],
...
if __name__ == "__main__":
    ...
    print(f"Gabor U-Net params    : {count_parameters(gabor_model):,}")

    base_val_m, base_val_r = evaluate_model(baseline_model, "baseline_unet", "validation")
    gabor_val_m, gabor_val_r = evaluate_model(gabor_model, "gabor_unet", "validation")

    base_test_m, base_test_r = evaluate_model(baseline_model, "baseline_unet", "test")
    gabor_test_m, gabor_test_r = evaluate_model(gabor_model, "gabor_unet", "test")

    print("\n=== Hard Sample Analysis (10 lowest Gabor U-Net Dice on test set) ===")
    ...
    for label, bm, gm in [("Validation", base_val_m, gabor_val_m),
        ...
        print(f"    Gabor U-Net     Dice={gm['mean_dice']:.4f}  IoU={gm['mean_iou']:.4f}  PixAcc={gm['pixel_accuracy']:.4f}")
